...
import { hullForEntity } from './Area.mjs';
 ...
 */
export function pointContents(p) {
  const cont = hullPointContents(SV.server.worldmodel.hulls[0], 0, p);
 ...
 */
export function recursiveHullCheck(hull, num, p1f, p2f, p1, p2, trace) {
  if (num < 0) {
  ...
  if (hullPointContents(hull, node.children[1 - side], mid) !== Defs.content.CONTENT_SOLID) {
 ...
 */
export function clipMoveToEntity(ent, start, mins, maxs, end) {
  const trace = {
    fraction: 1.0,
    allsolid: true,
    startsolid: false,
  ...
  const hull = hullForEntity(ent, mins, maxs, offset);
  recursiveHullCheck(hull, hull.firstclipnode, 0.0, 1.0, start.copy().subtract(offset), end.copy().subtract(offset), trace);
 ...
 */
export function clipToLinks(node, clip) {
 ...
 * @returns {Trace} collision result
 */
export function move(start, mins, maxs, end, type, passedict, ignoreedicts = []) {
  const clip = {
    trace: clipMoveToEntity(SV.server.edicts[0], start, mins, maxs, end),
    start,
    end,
    mins,
  ...
  clipToLinks(SV.areanodes[0], clip);
  return clip.trace;
}
 ...
 */
export function testEntityPosition(ent) {
  const origin = ent.entity.origin.copy();
  return move(origin, ent.entity.mins, ent.entity.maxs, origin, 0, ent).startsolid;
}
